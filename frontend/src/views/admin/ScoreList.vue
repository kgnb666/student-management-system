<template>
  <el-card class="page-card" shadow="never">
    <div class="toolbar">
      ...
      <el-select v-model="query.courseId" placeholder="全部课程" clearable filterable style="width: 190px">
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑成绩' : '新增成绩'" width="520px">
      <el-form :model="form" label-width="90px">
        <el-form-item label="课程" required>
          <el-select v-model="form.courseId" filterable style="width: 100%" @change="loadDialogStudents">
            <el-option v-for="item in courses" :key="item.id" :label="item.courseName" :value="item.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="期末成绩">
          <el-input-number v-model="form.examScore" :min="0" :max="100" :precision="1" style="width: 100%" />
        </el-form-item>
...
async function loadScores() {
  loading.value = true
  try {
    scores.value = await getScores(query)
  } finally {
    loading.value = false
  }
}
...
async function loadOptions() {
  const [courseList, semesterList] = await Promise.all([getCourses({}), getSemesters()])
